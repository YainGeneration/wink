...
sp = spotipy.Spotify(auth_manager=SpotifyOAuth(
    client_id='5ec7c7a102b44f489f1b88ef87d84d1c',
    client_secret='51d3ba3fa9084d4c84580b278046ae45',
...
df = df[df["year"].between(2013, 2014)]  # 다은
...
# 본인이 맡은 구간으로 프린트문 수정
print(f"🎧 총 {len(track_ids)}개의 트랙 ID를 불러왔습니다. (연도:2013~2014)\n")
...
for start in range(0, len(track_ids), batch_size):
    ...
    # 각자 맡은 연도 입력(파일 저장)
    temp_path = os.path.join("data/spotify_track_info_partial_(연도:2013~2014).csv")
    temp_df.to_csv(temp_path, index=False)
    print(f"💾 Batch {batch_num} 저장 완료 ({len(temp_df)}곡 누적)")

# ===== 4. 최종 저장 =====
# 각자 맡은 연도 입력(파일 저장)
output_path = os.path.join("data/spotify_track_info_after2000_(연도:2013~2014).csv")
...
# 각자 맡은 연도 입력(파일 저장)
if failed_ids:
    fail_path = os.path.join("data/failed_ids_after2000_(연도:2013~2014).csv")
