...
- Agent1~3: 텍스트/이미지 기반 추천
- Agent4: 위치 + 이미지 + 주변 음악 기반 추천
"""
...
try:
    ...
    print(f"❌ run_agent_pipeline 불러오기 실패: {e}. 'agent3_keywordExtractor.py' 파일 이름을 확인하세요.")
    exit()
...
def recommend():
    ...
    image_path = ""  # 일반 모드에서 생성되는 임시 파일 경로
    ...
    try:
        ...
        for song in recommended_raw:
            ...
            if track_id_full.startswith("track_"):
                try:
                    track_number = int(track_id_full.split("_")[-1])
